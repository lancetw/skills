"""Tests for the learn-tw Markdown -> readable-HTML converter.
...
# --- 自我檢核 (retrieval-practice) <details> disclosure --------------------------
# learn-tw is a *learning* doc, not a passive read: each section can end with a
# question the reader answers FROM MEMORY before revealing the answer (retrieval
# practice builds long-term retention far better than re-reading). The model writes
# it as a <details class="self-check"> so it collapses by default, its inner answer
# is parsed as real Markdown (md_in_html), and it degrades to a native disclosure
# on GitHub/GitLab.
...
def test_self_check_details_renders_as_styled_parsed_disclosure():
    # The disclosure must survive with its class, the <summary> prompt is kept, the
    # inner answer is parsed as a real Markdown list (so it is not a wall of raw
    # HTML), and the editorial styling exists so it is not a bare native triangle on
    # the warm-paper page.
    html = convert(SELF_CHECK_MD, timestamp="t")
    assert '<details class="self-check">' in html
    assert "<summary>自我檢核：資料經過哪幾層？</summary>" in html
    assert "<li>讀取層</li>" in html  # inner answer parsed as Markdown, not raw HTML
    assert "details.self-check" in html  # ...and the editorial styling is present


def test_self_check_answers_revealed_when_printed():
    # A printout is a reference the reader keeps; a collapsed <details> would print
    # with every answer hidden, making the printed self-checks useless. The print
    # stylesheet must force the answers open on paper.
    html = convert(SELF_CHECK_MD, timestamp="t")
    assert "details.self-check:not([open])" in html  # the print-reveal rule


# The canonical self-check (per SKILL.md) opens with a "> 先自己想，再展開對照。" nudge
# written as a Markdown blockquote right after the <summary>.
...
def test_self_check_hint_shows_while_collapsed_not_only_when_expanded():
    # Bug: the "先自己想，再展開對照。" nudge sat in the <details> BODY, so a closed card
    # hid it — yet the nudge's whole job is to be read BEFORE expanding ("think first,
    # then expand to compare"). It must be lifted into the always-visible <summary> so
    # it shows while collapsed, and must NOT remain a body blockquote that only
    # appears once the card is opened.
    html = convert(SELF_CHECK_WITH_HINT_MD, timestamp="t")
    summary = re.search(r"<summary>(.*?)</summary>", html, re.DOTALL).group(1)
    assert "先自己想，再展開對照。" in summary  # nudge lives in the (visible) summary
    assert '<span class="self-check-hint">' in summary
    assert "<blockquote>" not in html  # not left hidden in the body as a blockquote
    # the question itself is still there alongside the nudge
    assert "自我檢核：資料經過哪幾層？" in summary


def test_self_check_lifted_hint_is_dropped_once_expanded():
    # Once the card is open, "think first, then expand" is moot; CSS hides the lifted
    # nudge so it does not clutter the revealed answer.
    html = convert(SELF_CHECK_WITH_HINT_MD, timestamp="t")
    assert "details.self-check[open] > summary .self-check-hint" in html
...
def test_self_check_collapsed_marker_is_a_circled_question_mark():
    ...
    # an emoji glyph — so it renders in the seal-red accent and matches the editorial
    # page rather than a clashing colour emoji.
    html = convert(SELF_CHECK_MD, timestamp="t")
...
# A doc that ends with a 詞彙表 should let the reader click a term in the prose and
# see its definition float in place — so the appendix becomes interactive, not a
# scroll-away lookup. Terms are matched everywhere in the prose, click toggles the
# card, and the card shows 定義 + 避免混用.
GLOSSARY_MD = (
    "# T\n\n## 概述\n\n"
...
def test_glossary_term_in_prose_becomes_clickable_with_dashed_underline():
    # A prose occurrence is wrapped as a .gloss control carrying its glossary index,
    # and the dashed-underline + floating-card styles ship with it.
    html = convert(GLOSSARY_MD, timestamp="t")
    assert re.search(r'<button[^>]*class="gloss"[^>]*data-gloss="\d+"[^>]*>深類別</button>', html)
    assert "border-bottom: 1px dashed" in html  # the affordance
    assert ".gloss-pop" in html  # the floating card style


def test_glossary_all_occurrences_are_linked_not_just_the_first():
    # The reader chose "every occurrence": 深類別 appears twice in the prose, so both
    # must be clickable (not only the first). The prose is the first <p> of the body.
    html = convert(GLOSSARY_MD, timestamp="t")
    para = re.search(r"<p>.*?</p>", html, re.DOTALL).group(0)
    assert para.count('class="gloss"') >= 4  # 深類別 ×2 + classitis + 複雜度
    assert para.count(">深類別</button>") == 2


def test_glossary_definition_and_avoid_are_embedded_for_the_popover():
    html = convert(GLOSSARY_MD, timestamp="t")
    assert "/* glossary popover */" in html  # the click-to-toggle runtime is injected
    assert "介面窄、功能多的類別" in html  # the definition is shipped to the card
    assert "避免混用" in html  # the label
    assert "大類別" in html  # the 避免混用 note
...
def test_glossary_table_itself_is_not_linkified():
    # The definitions live in a <table>; linkifying there would self-reference and
    # nest controls. The term cells inside the table body must stay plain text.
    html = convert(GLOSSARY_MD, timestamp="t")
    tbody = re.search(r"<tbody>.*?</tbody>", html, re.DOTALL).group(0)
    assert "<td>深類別 (deep class)</td>" in tbody  # the cell is plain text
    assert 'class="gloss"' not in tbody


def test_glossary_does_not_linkify_headings():
    # Headings are skipped, so a term appearing in a heading stays plain.
    html = convert(GLOSSARY_MD, timestamp="t")
    assert re.search(r"<h2[^>]*>詞彙表</h2>", html)


def test_no_glossary_means_no_runtime_or_markup():
    # A doc without a 詞彙表 must not pay for the feature: no controls and no script.
    html = convert(SAMPLE_MD, timestamp="t")
...
def test_glossary_popover_caret_tracks_the_term():
    # The chosen look is a speech bubble: its caret must keep pointing at the term even
    # after the card is clamped to the viewport edge — so the caret offset is a CSS var
    # the runtime sets per-open, not a fixed position.
    html = convert(GLOSSARY_MD, timestamp="t")
    assert ".gloss-pop::before" in html  # the caret exists
    assert "--caret-left" in html  # ...its horizontal offset is variable
    assert "setProperty('--caret-left'" in html  # ...set dynamically on open
    # and it flips to the correct edge depending on open direction
