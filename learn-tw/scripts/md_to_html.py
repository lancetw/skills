#!/usr/bin/env python3
"""Convert a FOR[name].md learning document into a styled, readable HTML page.

This is the deterministic half of the learn-tw skill: the model writes the
Markdown, and this script turns it into a "好讀版" (easy-to-read) HTML file with
an editorial, book-like look (書卷感) tuned for long-form Traditional Chinese
reading.

Usage:
    python3 md_to_html.py FOR[name].md            # -> FOR[name].html
    python3 md_to_html.py FOR[name].md out.html   # explicit output path

Design notes:
- We reuse the `markdown` library (already a repo dependency in fcs-forensics)
  with the `tables`, `fenced_code`, and `toc` extensions. `toc` both injects
...
try:
    ...
    sys.stderr.write(
        ...
        "    uv sync                                      # 在此技能目錄安裝相依\n"
        "或直接用 uv 執行（會自動帶入相依）：\n"
        "    uv run python scripts/md_to_html.py <FOR[name].md>\n"
    )
    raise SystemExit(1)
...
# A learn-tw doc ends with a 詞彙表 (term | definition | 避免混用) table. Where each
# term appears in the prose, turn it into a dashed-underlined control that pops its
...
# into the learning doc; detect them and re-emit as a styled <div> callout. Runs
...
def convert(md_text: str, title: str = "學習文件", timestamp: str | None = None) -> str:
...
def main(argv: list[str]) -> int:
    if not argv:
        sys.stderr.write("用法：python3 md_to_html.py <FOR[name].md> [output.html]\n")
        return 2
