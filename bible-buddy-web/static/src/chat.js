...
import {
  ...
  Slot, mdHtml, local, useTicker, TOOL_ZH, Fragment,
} from './lib.js';
...
export function useChat({ openChat, onNote }) {
    ...
    for (;;) {
      ...
      while ((i = rest.indexOf('\n\n')) >= 0) {
        ...
        if (ev.type === 'text') {
          buf += ev.text + '\n';
          const t = buf;
          patch(botId, m => ({ ...m, text: t, act: '撰寫中…' }));
          setStatus('✍ 撰寫中…');
        } else if (ev.type === 'tool') {
          const z = TOOL_ZH[ev.name] || ev.name;
          setStatus(`⚙ ${z}…`);
          patch(botId, m => ({ ...m, act: `${z}…`, steps: [...(m.steps || []), `⚙ ${ev.name} ${ev.input}`] }));
        } else if (ev.type === 'note') {
          onNote(ev.note);
        } else if (ev.type === 'done') {
          finish();
          setStatus(`✓ 完成 · ${ev.turns} turns`);
          sendSlot.current?.flash?.('完成 ✓');
          push({ kind: 'tool', text: `✓ ${ev.turns} turns${ev.is_error ? ' · ERROR' : ''}` });
          const o = options(buf);
          if (o) push({ kind: 'opts', opts: o });
        } else if (ev.type === 'error') {
          finish();
          setStatus('✗ 出錯');
          push({ kind: 'tool', text: `✗ ${ev.text}` });
        }
      }
    }
    if (!finished) {
      finish();
      setStatus('✗ 連線中斷，重新整理可接回進行中的回答');
      push({ kind: 'tool', text: '✗ 串流在 done 之前結束（server 重啟？）' });
    }
  }, [onNote]);
  ...
  const send = useCallback(async text => {
    ...
    const botId = push({ kind: 'bot', text: '', steps: [], act: 'agent 啟動中…', pending: true, t0: Date.now() });
  ...
  const reattach = useCallback(async () => {
    ...
    openChat();
    setRunning(true);
    setStatus('⚙ agent 處理中（接回）…');
    const botId = push({ kind: 'bot', text: '', steps: [], act: '接回進行中的回答…', pending: true, t0: Date.now() });
  ...
  const stop = () => { setStatus('⏹ 停止中…'); fetch('/api/chat/stop', { method: 'POST' }); };
  ...
  return { messages, running, status, input, setInput, inputRef, sendSlot, send, stop, clear, reattach, note, prefillInput, toggleFold };
}
...
function Message({ m, onFold, onOption }) {
  ...
  if (m.kind === 'tool') return html`<div className="tool">${m.text}</div>`;
  if (m.kind === 'opts') return html`
    <div className="opts">
      ${m.opts.map(([n, t]) => html`<button key=${n} onClick=${() => onOption(n)}>${n}. ${t}</button>`)}
    </div>`;
  return html`<${Bubble} m=${m} />`;
}

export function Chat({ chat, onGutterDown }) {
  ...
  return html`
    <aside id="chat">
      ...
      <div id="log" ref=${log}>
        ${chat.messages.map(m => html`<${Message} key=${m.id} m=${m} onFold=${chat.toggleFold} onOption=${chat.send} />`)}
      </div>
      <div id="guide" hidden=${!empty}>
        ...
        <p><b>其他入口</b></p>
        <p>反白經文 → 手動筆記或 ELI5 筆記。<br />✨ 生成 ELI5 筆記：一次掃整段，寫成淺顯易懂的筆記；「未驗證」的可從卡片請 agent 驗證。<br />對話會保留到你按「清除」；換段經文會重新開始。</p>
      </div>
      <${Slot} tag="div" id="status" className="tool" text=${chat.status} />
      <form id="f" onSubmit=${e => {
        ...
        <button className=${`primary${chat.running ? ' stop' : ''}`} id="send" type="submit">
          <${Slot} text=${chat.running ? '停止' : '送出'} apiRef=${chat.sendSlot} />
        </button>
      </form>
    </aside>`;
}
